...
export function SkillBuddyChat({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div
      className={
        embedded
          ? "flex h-full min-h-[620px] flex-col overflow-hidden rounded-xl border bg-card soft-shadow"
    ...
    >
      <div className="flex items-center gap-3 border-b p-4">
        ...
        <div>
          <h2 className="font-display font-bold">SkillBuddy</h2>
          <p className="text-xs text-muted-foreground">Career intelligence assistant</p>
        </div>
      </div>
        ...
        <ConversationContent className="gap-5 p-5">
          {messages.map((message) => (
            <Message key={message.id} from={message.role}>
              <MessageContent
                className={message.role === "user" ? "bg-secondary text-secondary-foreground" : ""}
              >
          ...
          {status === "submitted" && (
            <Message from="assistant">
              <MessageContent>
                <Shimmer>Connecting your skills to industry data...</Shimmer>
              </MessageContent>
            </Message>
          )}
          <ConversationScrollButton />
      ...
      <div className="border-t p-4">
        ...
        <PromptInput onSubmit={({ text }) => send(text)}>
          <PromptInputTextarea placeholder="Ask about your career..." className="min-h-20" />
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      <div
        className={
          open
            ? "mb-3 h-[min(680px,calc(100vh-7rem))] w-[min(410px,calc(100vw-2rem))] overflow-hidden rounded-xl border bg-card lift-shadow"
            : "hidden"
        }
      >
      ...
      <div className="flex items-center justify-end gap-3">
        <Link
          to="/ai-assistant"
          className="hidden rounded-full border bg-card px-4 py-2 text-sm font-medium shadow-sm md:block"
        >
          Ask SkillBuddy about your roadmap
        </Link>
        <Button
