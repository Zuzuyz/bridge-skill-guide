...
/* =========================================================
   PHASE 13 — COLLEGE ANALYTICS CORE (SERVER-ONLY)
   ---------------------------------------------------------
   ...
   This module is SERVER-ONLY (imports src/server/db.server).
   The client-safe wrapper college-analytics-server.ts loads
   it via dynamic import inside its createServerFn handler —
   the same pattern as matching-server.ts /
...
export async function resolveCollegeForSession(): Promise<CollegeResolution> {
...
export function studentScope(collegeName: string) {
  return {
...
export async function computeCollegeAnalytics(): Promise<CollegeAnalyticsResult> {
